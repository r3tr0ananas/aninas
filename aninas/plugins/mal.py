...
async def mal_command(
    ...
    query: str,
    search_type: str = commands.Param(
        "anime",
        description="What should be searched, defaults to anime.",
        choices=["anime", "character"],
    ),
):
...
def character_embed(data: Character) -> disnake.Embed:
    embed = disnake.Embed(title=data.name, description=data.about)
    ...
    embed.add_field(name="Nicknames", value=" | ".join(data.nicknames))

    embed.set_thumbnail(data.image.url)

    return embed


def anime_embed(data: Anime) -> disnake.Embed:
    embed = disnake.Embed(title=data.title, description=data.data["data"]["synopsis"])
    ...
    if data.type == "TV":
        ...
        last_air = (
            ...
            if data.aired.to and data.aired.from_ != data.aired.to
            else f"`{data.status.value}`"
        )
