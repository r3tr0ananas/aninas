    ...
    async def callback(self, inter: disnake.MessageInteraction) -> None:
        if self.author.id != inter.author.id:
            return await inter.response.send_message(
                "You are not allowed to press this button", ephemeral=True
            )
...
async def search(
    inter: disnake.CommandInteraction, query: str = commands.Param(description="Query")
):
    await inter.response.defer()
    ...
    metadata = await tmdb.get((results[0]["id"], results[0]["type"]))
...
def person_embed(metadata: Person) -> disnake.Embed:
    ...
    embed = disnake.Embed(title=name, description=metadata.bio)
...
def media_embed(metadata: Movie | TV) -> disnake.Embed:
    ...
    embed = disnake.Embed(title=name, description=metadata.overview)

    embed.add_field(
        f"{Emojis.information} Info",
        line_fix(
            f"""
    ...
    if isinstance(metadata, TV):
        ...
        last_air_date = (
            ...
            if metadata.first_date
            else "Didn't start yet"
        )
        next_air_date = (
            f"<t:{int(metadata.next_date.timestamp())}:d>"
            if metadata.next_date
            else ("Not set/Show ended" if metadata.first_date else "No idea")
        )

        embed.add_field(
            f"{Emojis.tv} Show Info",
            line_fix(
                f"""
