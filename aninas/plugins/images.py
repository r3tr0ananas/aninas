...
async def quote(inter: disnake.ApplicationCommandInteraction, message: disnake.Message):
    if len(message.content) > 30:
        return await inter.response.send_message(
            "Message is too long to quote", ephemeral=True
        )

    await inter.response.defer()

    author_image = io.BytesIO(httpx.get(message.author.avatar.url).content)

    author_image = Image.open(author_image)

    img_buf = io.BytesIO()

    make_it_quote(author_image, message.content, message.author.name).save(
        img_buf, format="png"
    )
...
async def this_is(
    ...
    make_this_is(who, image).save(img_buf, format="png")

    img_buf.seek(0)

    await inter.followup.send(file=disnake.File(img_buf, f"{who}.png"))
