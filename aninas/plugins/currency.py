...
@plugin.slash_command(
    description="Convert currency from one currency to another. Exchange rates provided by the ECB."
)
...
async def convert(
    ...
    target: str = commands.Param(
        "USD", description="The currency you're converting to. Defaults to USD."
    ),
    date: str = commands.Param(
        None,
        description="Optional date to fetch historical exchange rates. E.G: 2014-2-24",
    ),
):
    if date is not None:
